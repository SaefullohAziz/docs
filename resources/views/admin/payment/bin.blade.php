	...
	<div class="col-12">
		...
		<div class="card card-primary">
			...
			<div class="card-footer bg-whitesmoke">
				@if (auth()->guard('admin')->user()->can('restore payments'))
					<button class="btn btn-warning btn-sm" name="restoreData" title="{{ __('Restore') }}">{{ __('Restore') }}</button>
				@endif
				@if (auth()->guard('admin')->user()->can('force_delete payments'))
...
<script>
	...
	$(document).ready(function() {
		table = $('#table4data').DataTable({
			...
			serverSide: true,
			"ajax": {
				"url": "{{ route('admin.payment.binList') }}",
				"type": "POST",
				"data": function (d) {
		          d._token = "{{ csrf_token() }}";
		        }
        ...
        $('[name="restoreData"]').click(function(event) {
            if ($('[name="selectedData[]"]:checked').length > 0) {
                ...
                .then((willRestore) => {
                    if (willRestore) {
                        $.ajax({
                            url : "{{ route('admin.payment.restore') }}",
                            type: "POST",
                            dataType: "JSON",
        ...
        $('[name="deleteData"]').click(function(event) {
            if ($('[name="selectedData[]"]:checked').length > 0) {
                ...
                .then((willDelete) => {
                    if (willDelete) {
                        $.ajax({
                            url : "{{ route('admin.payment.destroyPermanently') }}",
                            type: "DELETE",
                            dataType: "JSON",
                            data: {"selectedData" : selectedData, "_token" : "{{ csrf_token() }}"},
                            success: function(data)

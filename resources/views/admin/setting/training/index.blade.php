                                            ...
                                            <div class="tab-pane fade {{ ($loop->first?'active show':'') }}" id="form-setting-{{ strtolower(str_replace(' ', '-', $form->slug)) }}" role="tabpanel" aria-labelledby="form-setting-{{ $loop->iteration }}-tab">
                                                ...
                                                <fieldset class="{{ $form->limiter_slug }}-set {{ (setting($form->status_slug)==1?'d-block':'d-none') }}">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        {{ Form::bsText('col-12', __('Additional Price (Per Participant)'), $form->more_participant_slug, setting($form->more_participant_slug), __('Exc : 2000000'), [(setting($form->status_slug)==1?'required':'') => ''], [__("Leave blank to deactivate additional participants or fill in zero to make it free.")]) }}

					...
					<div class="row">
                        <div class="col-sm-6">
                            <fieldset>
                                ...
                                {{ Form::bsSelect(($data->type=='Visitasi'?'d-block':'d-none'), __('Destination'), 'destination', $destinations, $data->destination, __('Select'), ['placeholder' => __('Select')]) }}

                                {{ Form::bsCheckboxList(($data->type=='Visitasi'?'d-block':'d-none'), __('Participant'), 'participant[]', $participants, (empty($data->participant)?[]:explode(', ', $data->participant))) }}

                                {{ Form::bsSelect(($data->type=='Audiensi'?'d-block':'d-none'), __('Transportation'), 'transportation', $transportations, $data->transportation, __('Select'), ['placeholder' => __('Select')]) }}
                            </fieldset>
                            <fieldset class="{{ ($data->type=='Audiensi'?'d-block':'d-none') }}">
								<legend>{{ __('Participant') }}</legend>
								{{ Form::bsSelect(null, __('Participant'), 'select_participant', [], $data->select_participant, __('Select'), ['placeholder' => __('Select'), 'disabled' => '']) }}
								<fieldset>
                        ...
                        <div class="col-sm-6">
							<fieldset class="{{ ($data->type=='Audiensi'?'d-block':'d-none') }}">
								...
								{{ Form::bsText(($data->type=='Audiensi'?'d-block':'d-none'), __('Date'), 'date', (empty($data->date)?null:date('d-m-Y', strtotime($data->date))), __('Date'), []) }}

								{{ Form::bsSelect(($data->type=='Audiensi'?'d-block':'d-none'), __('Arrival Point'), 'arrival_point', $arrivalPoints, $data->arrival_point, __('Select'), ['placeholder' => __('Select')]) }}

								{{ Form::bsSelect(($data->type=='Audiensi'?'d-block':'d-none'), __('Contact Person'), 'contact_person', $contactPersons, optional($contactPerson)->id, __('Select'), ['placeholder' => __('Select')]) }}
							</fieldset>
							<fieldset class="{{ ($data->type=='Audiensi'?'d-block':'d-none') }}">
								<legend>{{ __('Return Information') }}</legend>
								{{ Form::bsText(($data->type=='Audiensi'?'d-block':'d-none'), __('Until Date'), 'until_date', (empty($data->until_date)?null:date('d-m-Y', strtotime($data->until_date))), __('Until Date'), []) }}
							</fieldset>
							<fieldset>
								<legend></legend>
								{{ Form::bsFile(($data->type=='Visitasi'?'d-block':'d-none'), __('Submission Letter'), 'submission_letter', old('submission_letter'), [], [__('File with PDF/JPG/PNG format up to 5MB.')]) }}
							</fieldset>
                        </div>
					</div>
				</div>

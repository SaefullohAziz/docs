	...
	<div class="col-12">
		...
		<div class="card card-primary">
			<div class="card-header">
				@if(auth()->guard('admin')->user()->can('create attendances'))
					<a href="{{ route('admin.attendance.create') }}" class="btn btn-icon btn-success" title="{{ __('Create') }}"><i class="fa fa-plus"></i></a>
				@endif
				...
				@if(auth()->guard('admin')->user()->can('bin attendances'))
					<a href="{{ route('admin.attendance.bin') }}" class="btn btn-icon btn-danger" title="{{ __('Bin') }}"><i class="fas fa-trash"></i></a>
				@endif
			</div>
			<div class="card-body">
			...
			<div class="card-footer bg-whitesmoke">
				@if (auth()->guard('admin')->user()->can('approval attendances'))
				...
				@endif
				@if (auth()->guard('admin')->user()->can('delete attendances'))
	...
	$(document).ready(function() {
		table = $('#table4data').DataTable({
			...
			columns: [
				...
				{ data: 'type', name: 'attendances.type' },
				{ data: 'submission_letter', name: 'attendances.submission_letter' },
		...
		$('[name="deleteData"]').click(function(event) {
			if ($('[name="selectedData[]"]:checked').length > 0) {
			    ...
			    .then((willDelete) => {
			      	if (willDelete) {
			      		$.ajax({
							url : "{{ route('admin.attendance.destroy') }}",
							type: "DELETE",
							dataType: "JSON",

	...
	<div class="col-12">
		...
		<div class="card card-primary">
			<div class="card-header">
				<a href="{{ route('admin.attendance.index') }}" class="btn btn-icon btn-success" title="{{ __('Back') }}"><i class="fas fa-chevron-left"></i></a>
			...
			<div class="card-footer bg-whitesmoke">
				@if (auth()->guard('admin')->user()->can('restore attendances'))
					<button class="btn btn-warning btn-sm" name="restoreData" title="{{ __('Restore') }}">{{ __('Restore') }}</button>
				@endif
				@if (auth()->guard('admin')->user()->can('force_delete attendances'))
...
<script>
	...
	$(document).ready(function() {
		table = $('#table4data').DataTable({
			...
			serverSide: true,
			"ajax": {
				"url": "{{ route('admin.attendance.binList') }}",
				"type": "POST",
				"data": function (d) {
		          d._token = "{{ csrf_token() }}";
		        }
		...
		$('[name="restoreData"]').click(function(event) {
			if ($('[name="selectedData[]"]:checked').length > 0) {
			    ...
			    .then((willRestore) => {
			      	if (willRestore) {
			      		$.ajax({
							url : "{{ route('admin.attendance.restore') }}",
							type: "POST",
							dataType: "JSON",
		...
		$('[name="deleteData"]').click(function(event) {
			if ($('[name="selectedData[]"]:checked').length > 0) {
			    ...
			    .then((willDelete) => {
			      	if (willDelete) {
			      		$.ajax({
							url : "{{ route('admin.attendance.destroyPermanently') }}",
							type: "DELETE",
							dataType: "JSON",
							data: {"selectedData" : selectedData, "_token" : "{{ csrf_token() }}"},
							success: function(data)
